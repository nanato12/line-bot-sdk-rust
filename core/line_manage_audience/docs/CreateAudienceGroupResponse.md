...
**create_route** | Option<**CreateRoute**> | How the audience was created.  `MESSAGING_API`: An audience created with Messaging API.  (enum: MESSAGING_API) | [optional]
...
**permission** | Option<**Permission**> | Audience's update permission. Audiences linked to the same channel will be READ_WRITE.  `READ`: Can use only. `READ_WRITE`: Can use and update.  (enum: READ, READ_WRITE) | [optional]
