...
pub trait MessagingApiBlobApi {
    fn get_message_content(&self, message_id: &str) -> Pin<Box<dyn Future<Output = Result<std::path::PathBuf, Error>>>>;
    fn get_message_content_preview(&self, message_id: &str) -> Pin<Box<dyn Future<Output = Result<std::path::PathBuf, Error>>>>;
    fn get_message_content_transcoding_by_message_id(&self, message_id: &str) -> Pin<Box<dyn Future<Output = Result<crate::models::GetMessageContentTranscodingResponse, Error>>>>;
    fn get_rich_menu_image(&self, rich_menu_id: &str) -> Pin<Box<dyn Future<Output = Result<std::path::PathBuf, Error>>>>;
    fn set_rich_menu_image(&self, rich_menu_id: &str, body: Option<std::path::PathBuf>) -> Pin<Box<dyn Future<Output = Result<(), Error>>>>;
}
