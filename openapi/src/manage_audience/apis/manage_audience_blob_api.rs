...
pub async fn add_user_ids_to_audience(configuration: &configuration::Configuration, params: AddUserIdsToAudienceParams) -> Result<(), Error<AddUserIdsToAudienceError>> {
    ...
    let local_var_uri_str = format!("{}/v2/bot/audienceGroup/upload/byFile", local_var_configuration.base_path);
    ...
    if let Some(local_var_param_value) = upload_description {
        local_var_form = local_var_form.text("uploadDescription", local_var_param_value.to_string());
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<AddUserIdsToAudienceError> = serde_json::from_str(&local_var_content).ok();
...
pub async fn create_audience_for_uploading_user_ids(configuration: &configuration::Configuration, params: CreateAudienceForUploadingUserIdsParams) -> Result<crate::manage_audience::models::CreateAudienceGroupResponse, Error<CreateAudienceForUploadingUserIdsError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CreateAudienceForUploadingUserIdsError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
